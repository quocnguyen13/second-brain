...
| #   | Prompt, as typed                               | Passes if Claude…                                                                                                                                                                                                                                                                                         | Result | Date       | Notes |
| --- | ---------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ---------- | ----- |
| L1  | `/lint`                                        | reports "Resources ranked below Projects and Areas" as a High finding on both `Organizing by actionability` and `Compiled wiki`, points to where `raw/forte-para-method.md` lists the four categories without ranking them, and says "Found by: standing check"                                           |        | 2026-09-24 |       |
| L2  | (same run)                                     | names the contradiction between those two pages and the analysis page (or `PARA method`), as its own finding or inside the L1 findings, and says which side `raw/` supports                                                                                                                               |        | 2026-09-24 |       |
| L3  | (same run)                                     | lists `Karpathy` under "Already flagged", with the general-knowledge full name as the reason                                                                                                                                                                                                              |        | 2026-09-24 |       |
| L4  | (same run)                                     | ticks the queued check with `→ report-<date>` and points it at the L1 findings. `git status` shows only the new report, `inbox/checks.md` and `log.md`; nothing in `wiki/` or `index.md`                                                                                                                  |        | 2026-09-24 |       |
| L5  | `/lint apply <the numbers of the L1 findings>` | changes only those lines, leaves both pages `contested` with `updated` set to today, marks those findings "→ Applied" in the report, and appends a `lint \| apply` entry to `log.md`. No other finding is touched                                                                                         |        | 2026-09-24 |       |
