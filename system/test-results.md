...
| # | Prompt, as typed | Passes if Claude… | Result | Date | Notes |
|---|---|---|---|---|---|
| R1 | `/drafts` | checks all 11 drafts: each gets a `## Check <date>` section at the end and `checked: <date>` in its properties. `git diff` shows nothing else changed in any draft (title, text, Relations, `status`, `origin`), and `git status` shows only `mine/drafts/` and `log.md` | Pass | 2026-09-25 | Commit 45526e4. All 11 drafts got a Check section and `checked: "2026-09-25"`; the diff adds lines only, none removed. The commit holds only `mine/drafts/` and `log.md` (your `system/context.md` edit went in 70cf510). Log: 11 checked, 2 clean, 9 with problems. |
...
**Score so far:** 4 of 4 (R1–R4). R5 runs after your first cycle through the Draft queue.

**Beyond the expected findings (2026-09-25).** The run found more than R1–R4 asked for, and Claude checked each one against `raw/` in the project chat. All of them hold:
- `The compiled wiki is a memex...` calls its main point "not a claim either source makes", but Karpathy makes it: Bush "couldn't solve ... who does the maintenance. The LLM handles that." (`raw/karpathy-llm-wiki.md` line 81).
- `The index file will need...` offers qmd as the alternative to embedding-based search, but qmd is "hybrid BM25/vector search" (line 64).
- `Exact-match search...` leaves out that Anthropic recommends combining the two methods, "Embeddings+BM25 is better than embeddings on their own", rather than ranking them (lines 52, 172–177).
